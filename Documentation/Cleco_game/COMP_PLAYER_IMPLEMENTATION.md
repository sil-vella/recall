...
**File**: `playbooks/rop01/09_setup_apps_database_structure.yml`

- Add `is_comp_player: false` to all existing dummy users in the users collection
- Add `is_comp_player: true` to at least 3-4 new dummy computer player users
- Add index: `db.users.createIndex({ "is_comp_player": 1 })`
...
- Randomly selects `count` users from results
...
1. `playbooks/rop01/09_setup_apps_database_structure.yml` - Add `is_comp_player` field
2. `python_base_04/core/modules/cleco_game/cleco_game_main.py` - Add endpoint and method
3. `dart_bkend_base_01/lib/services/python_api_client.dart` - Add `getCompPlayers()` method
4. `dart_bkend_base_01/lib/modules/cleco_game/backend_core/coordinator/game_event_coordinator.dart` - Update `_handleStartMatch()`
...
- [ ] Add `is_comp_player` field to users collection in database setup playbook
- [ ] Create `/public/cleco/get-comp-players` endpoint in Flask backend
- [ ] Implement `get_comp_players()` method with database query and random selection
- [ ] Add `getCompPlayers()` method to PythonApiClient
- [ ] Update `_handleStartMatch()` to fetch comp players for multiplayer (exclude practice mode)
- [ ] Implement fallback to simulated CPU players when API fails or insufficient comp players
