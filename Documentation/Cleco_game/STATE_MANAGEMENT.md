# State Management Documentation - Cleco Game (Flutter)

## Overview

This document describes the complete state management system for the Cleco game in Flutter. It covers state architecture, update flows, player actions, event handling, widget synchronization, and all state-related scenarios during gameplay.
...
2. **ClecoGameStateUpdater** (Singleton)
...
4. **ClecoEventHandlerCallbacks** (Static methods)
...
StateQueueValidator.validateUpdate() & enqueue
    ↓
ClecoGameStateUpdater._applyValidatedUpdates()
    ↓
...
The Cleco game module state is stored under the key `'cleco_game'` in `StateManager`. The complete structure is:
...
2. **Event Routing**
   - `ClecoEventManager.handleGameStateUpdated()` receives event
   - Delegates to `ClecoEventHandlerCallbacks.handleGameStateUpdated()`
...
6. **Widget Slice Computation**
   - `ClecoGameStateUpdater._applyValidatedUpdates()` called
...
1. **`ClecoGameHelpers.updateUIState()`** (Most Common)
   - Main path for UI updates
   - Goes through `ClecoGameStateUpdater.updateState()`
   - Validated and queued (async)
   - Used for most state updates

2. **`ClecoGameStateUpdater.updateStateSync()`** (Critical Flags)
   - Synchronous updates
...
5. Backend Processing
   Backend (ClecoGameRound) processes action
   - Validates action
   - Updates game state
...
**Special Actions:**
- `callCleco`: Call Cleco to end game
...
1. **`game_state_updated`** (Most Common)
   ...
   - Handler: `ClecoEventHandlerCallbacks.handleGameStateUpdated()`

2. **`game_state_partial_update`**
   - Partial game state update
   - Contains only changed fields
   - More efficient for small updates
   - Handler: `ClecoEventHandlerCallbacks.handleGameStatePartialUpdate()`

3. **`player_state_updated`**
   - Single player state update
   - Contains player-specific data
   - Handler: `ClecoEventHandlerCallbacks.handlePlayerStateUpdated()`

4. **`game_started`**
   - Game has started
   - Triggered when match begins
   - Handler: `ClecoEventHandlerCallbacks.handleGameStarted()`

5. **`turn_started`**
   - New turn has started
   - Contains turn information
   - Handler: `ClecoEventHandlerCallbacks.handleTurnStarted()`

6. **`cleco_new_player_joined`**
   - Player joined the room
   - Handler: `ClecoEventHandlerCallbacks.handleClecoNewPlayerJoined()`

7. **`cleco_joined_games`**
   - List of games user has joined
   - Handler: `ClecoEventHandlerCallbacks.handleClecoJoinedGames()`
...
6. **If timer expires:**
   ...
   - `ClecoGameHelpers.removePlayerFromGame(gameId)` called
7. State cleanup:
...
5. **If room becomes empty:**
   ...
   - Backend disposes `ClecoGameRound` instance
   - Game state cleared
6. **Frontend:**
...
**Method: `ClecoGameHelpers.removePlayerFromGame()`**
