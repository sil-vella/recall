...
This document describes the complete timer system and game leaving logic in the Cleco game. It covers all scenarios including user-initiated leaves, app closures, auto-leave on missed actions, and the differences between practice and multiplayer modes.
...
The Cleco game uses several types of timers to manage game flow and player actions:

#### 1. Draw Action Timer
- **Purpose**: Limits time for a player to draw a card
- **Duration**: Configurable via `turnTimeLimit` (default: 30 seconds)
- **Location**: `ClecoGameRound._drawActionTimer`
- **When Started**: 
...
- **Duration**: Configurable via `turnTimeLimit` (default: 30 seconds)
- **Location**: `ClecoGameRound._playActionTimer`
- **When Started**: 
...
- **Location**: `ClecoGameRound._sameRankTimer`
- **When Started**: 
...
- **Location**: `ClecoGameRound._specialCardTimer`
- **When Started**: 
...
- **Location**: `ClecoGameRound._shouldStartTimer()`
...
- **Location**: `ClecoGameRound._missedActionCounts` (Map<String, int>)
...
3. **Backend Processing** (`ClecoGameModule._onLeaveRoom()`):
...
3. **Room Cleanup** (`ClecoGameModule._onRoomClosed()`):
   - Disposes `ClecoGameRound` instance
   - Clears game state from `GameStateStore`
   - All timers are cancelled when `ClecoGameRound.dispose()` is called
...
**Flow**:
1. **Timer Expiry** (`ClecoGameRound._onDrawActionTimerExpired()` or `_onPlayActionTimerExpired()`):
...
2. **Threshold Reached** (`ClecoGameRound._onMissedActionThresholdReached()`):
...
4. **Leave Room Hook** (`ClecoGameModule._onLeaveRoom()`):
...
2. **Room Closed Hook** (`ClecoGameModule._onRoomClosed()`):
   - Disposes `ClecoGameRound` instance (cancels all timers)
   - Clears game state from `GameStateStore`
...
All timers in `ClecoGameRound` are cancelled when:
...
**Storage**: `Map<String, int> _missedActionCounts` in `ClecoGameRound`
...
**When Room Closes**:
1. `ClecoGameRound.dispose()` called (cancels all timers)
