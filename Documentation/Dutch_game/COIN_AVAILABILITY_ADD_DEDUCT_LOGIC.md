...
This document describes the comprehensive coin availability validation system for the Dutch game, which ensures users have sufficient coins before they can create or join multiplayer games.
...
2. **Helper Methods:**
   - `DutchGameHelpers.checkCoinsRequirement()` - Validates coins before create/join
   - `DutchGameHelpers.deductGameCoins()` - Deducts coins from all players when game starts
...
4. **Coin Deduction Trigger**
   - `DutchEventHandlerCallbacks._handleCoinDeductionOnGameStart()` - Triggered when game phase changes to `initial_peek`
...
2. If `fetchFromAPI` is true:
   - Calls `getUserDutchGameData()` to fetch fresh stats from API
...
3. If `fetchFromAPI` is false:
   - Uses cached state from `getUserDutchGameStats()`
...
final hasEnough = await DutchGameHelpers.checkCoinsRequirement();
...
Future<void> _createRoom(Map<String, dynamic> roomSettings) async {
  ...
  final hasEnoughCoins = await DutchGameHelpers.checkCoinsRequirement(
...
5. **Call API:** `DutchGameHelpers.deductGameCoins()` with all player user IDs
...
2. Falls back to cached state from `getUserDutchGameStats()`
...
```
📊 DutchGameHelpers: Fetching fresh user stats from API for coin check
📊 DutchGameHelpers: Fetched coins from API: 100
✅ DutchGameHelpers: Coins check passed - Required: 25, Current: 100

⚠️ DutchGameHelpers: Insufficient coins - Required: 25, Current: 10
⚠️ DutchGameHelpers: Failed to fetch stats from API, falling back to state
