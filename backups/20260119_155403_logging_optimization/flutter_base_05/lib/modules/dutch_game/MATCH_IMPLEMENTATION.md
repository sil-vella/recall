## Dutch Match Screen – Flutter Implementation Plan

This document details the architecture and delivery plan for the Dutch match screen and related UI/logic, aligned with the Python backend `dutch_*` WebSocket events and the existing Flutter Base 05 patterns.

### Goals
- Build the match (gameplay) screen UI and client-side logic on top of existing managers (`DutchGameManager`, `DutchStateManager`, `StateManager`, `WebSocketManager`).
- Align with backend event names and payloads (`dutch_*`) for clean, testable integration.
...
- Route: `/dutch/game-play` (registered in `DutchGameCore._registerScreens()` once ready).
- Base class: extend `BaseScreen` → `DutchMatchScreen` and `_DutchMatchScreenState`.
...
- Action bar: context-driven (Draw, Take from Discard, Play, Replace, Call Dutch, Use Power).
...
Managers:
- `DutchGameManager`: orchestrates high-level flows; already listens to WS and updates `DutchStateManager`.
- `DutchStateManager`: holds current `GameState`; exposes streams for turn/hand/phase.
- `StateManager`: global store to render `MessageBoardWidget` and lobby info.

Event Registration (receive):
- Subscribe via `WebSocketManager`/`WSEventManager` to `dutch_*` events:
  - `dutch_game_state_updated` → update full `GameState` in `DutchStateManager`.
  - `dutch_turn_changed` (or part of `game_state_updated`) → update current player.
  - `dutch_card_played`, `dutch_card_drawn` → per-action UI updates/animations.
  - `dutch_special_power_used` → trigger power dialogs result handling.
  - `dutch_dutch_called` → enter dutch phase, show banners/timers.
  - `dutch_game_ended` → show results dialog/panel.
  - `dutch_error` → toast/snackbar + session message board entry.

Event Emission (send):
- `dutch_join_game`, `dutch_leave_game` managed by lobby → reused if needed.
- `dutch_player_action` payloads:
...
Opponents Panel:
- Show each opponent’s name, card count, last action icon, dutch-called badge.
...
- Buttons enabled by rule state and `DutchStateManager`:
  - Draw (face-down), Take from Discard
  - Play, Replace, Place Drawn & Play, Place Drawn & Replace
  - Call Dutch, Use Power (with dialog)
- Semantics: `match_action_draw`, `match_action_take_discard`, `match_action_play`, `match_action_replace`, `match_action_call_dutch`, `match_action_use_power`.
...
From `DutchStateManager` derive:
- `isMyTurn`, `canCallDutch`, `phase`, `turnNumber`, `roundNumber`, `currentPlayerId`.
- Current hand, playable cards (backend determines playable cards, frontend only displays).
- Disable UI actions when not permitted; attempt anyway → server returns `dutch_error` to show message.
...
- On `dutch_error`: show snackbar + add session message via `DutchMessageManager`.
...
- Integration (manual + CDP console): semantics-driven end-to-end script (open drawer, navigate, join, draw, play, dutch).
...
- Add `/dutch/game-play` screen with header/center/hand skeleton.
- Wire event listeners (`dutch_*`) and update `DutchStateManager` mappings.
...
Phase D – Dutch and Results
- Implement Call Dutch flow; final round state; results panel.
...
- Confirm payload contracts for:
  - `dutch_game_state_updated`, `dutch_card_played`, `dutch_card_drawn`, `dutch_turn_changed`, `dutch_dutch_called`, `dutch_special_power_used`, `dutch_game_ended`, `dutch_error`.
- Confirm action payloads accepted under `dutch_player_action` for all listed actions.
