...
class ComputerPlayerFactory {
  ...
  Map<String, dynamic> getPlayCardDecision(String difficulty, Map<String, dynamic> gameState, List<String> availableCards) {
    _logger.info('Cleco: DEBUG - getPlayCardDecision called with difficulty: $difficulty, availableCards: ${availableCards.length}', isOn: LOGGING_SWITCH);
    
    final decisionDelay = config.getDecisionDelay(difficulty);
    final cardSelection = config.getCardSelectionStrategy(difficulty);
    final evaluationWeights = config.getCardEvaluationWeights();
    
    _logger.info('Cleco: DEBUG - Card selection strategy: $cardSelection', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Evaluation weights: $evaluationWeights', isOn: LOGGING_SWITCH);
    
    if (availableCards.isEmpty) {
      _logger.warning('Cleco: DEBUG - No cards available to play', isOn: LOGGING_SWITCH);
      return {
        'action': 'play_card',
        'card_id': null,
        'delay_seconds': decisionDelay,
    ...
    _logger.info('Cleco: DEBUG - Available cards: $availableCards', isOn: LOGGING_SWITCH);
    
    // Select card based on strategy
    final selectedCard = _selectCard(availableCards, cardSelection, evaluationWeights, gameState);
    
    _logger.info('Cleco: DEBUG - Selected card: $selectedCard', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> getJackSwapDecision(String difficulty, Map<String, dynamic> gameState, String playerId) {
    _logger.info('Cleco: DEBUG - getJackSwapDecision called with difficulty: $difficulty, playerId: $playerId', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - YAML strategy rules count: ${strategyRules.length}', isOn: LOGGING_SWITCH);
    if (strategyRules.isNotEmpty) {
      _logger.info('Cleco: DEBUG - YAML rules: ${strategyRules.map((r) => r['name']).join(', ')}', isOn: LOGGING_SWITCH);
    }
    ...
    _logger.info('Cleco: DEBUG - shouldPlayOptimal: $shouldPlayOptimal', isOn: LOGGING_SWITCH);
    
    // If no strategy rules defined, fallback to simple decision
    if (strategyRules.isEmpty) {
      _logger.info('Cleco: DEBUG - No YAML rules defined, using fallback logic', isOn: LOGGING_SWITCH);
      return {
        'action': 'jack_swap',
        'use': false,
        'delay_seconds': decisionDelay,
    ...
    _logger.info('Cleco: DEBUG - YAML rules engine returned decision: $decision', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> getQueenPeekDecision(String difficulty, Map<String, dynamic> gameState, String playerId) {
    _logger.info('Cleco: DEBUG - getQueenPeekDecision called with difficulty: $difficulty, playerId: $playerId', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> getCollectFromDiscardDecision(String difficulty, Map<String, dynamic> gameState, String playerId) {
    _logger.info('Cleco: DEBUG - getCollectFromDiscardDecision called with difficulty: $difficulty, playerId: $playerId', isOn: LOGGING_SWITCH);
    ...
    if (discardPile.isNotEmpty) {
      ...
      if (topCard != null) {
        ...
        _logger.info('Cleco: DEBUG - Added discard pile top card: ${topCard['rank']} of ${topCard['suit']}', isOn: LOGGING_SWITCH);
      }
    }
  ...
  String _selectCard(List<String> availableCards, Map<String, dynamic> cardSelection, Map<String, double> evaluationWeights, Map<String, dynamic> gameState) {
    _logger.info('Cleco: DEBUG - _selectCard called with ${availableCards.length} available cards', isOn: LOGGING_SWITCH);
    
    final strategy = cardSelection['strategy'] ?? 'random';
    _logger.info('Cleco: DEBUG - Strategy: $strategy', isOn: LOGGING_SWITCH);
    
    // Get current player from game state
    final currentPlayer = gameState['currentPlayer'] as Map<String, dynamic>?;
    if (currentPlayer == null) {
      _logger.warning('Cleco: DEBUG - No current player found, using random fallback', isOn: LOGGING_SWITCH);
      return availableCards[_random.nextInt(availableCards.length)];
    }
    
    _logger.info('Cleco: DEBUG - Current player: ${currentPlayer['name']}', isOn: LOGGING_SWITCH);
    
    // Prepare game data for YAML rules engine
    final gameData = _prepareGameDataForYAML(availableCards, currentPlayer, gameState);
    _logger.info('Cleco: DEBUG - Game data prepared: ${gameData.keys.join(', ')}', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Available cards: ${gameData['available_cards']}', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Playable cards: ${gameData['playable_cards']}', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Unknown cards: ${gameData['unknown_cards']}', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Known cards: ${gameData['known_cards']}', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Collection cards: ${gameData['collection_cards']}', isOn: LOGGING_SWITCH);
    ...
    if (strategyRules.isEmpty) {
      _logger.info('Cleco: DEBUG - No YAML rules defined, using legacy logic', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Optimal play probability: $optimalPlayProb, Should play optimal: $shouldPlayOptimal', isOn: LOGGING_SWITCH);
    
    // Execute YAML rules
    final rulesEngine = YamlRulesEngine();
    final result = rulesEngine.executeRules(strategyRules, gameData, shouldPlayOptimal);
    
    _logger.info('Cleco: DEBUG - YAML rules engine returned: $result', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _prepareGameDataForYAML(List<String> availableCards, 
    ...
    _logger.info('Cleco: DEBUG - _prepareGameDataForYAML called with ${availableCards.length} available cards', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Player known_cards: $knownCards', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Player collection_rank_cards: ${collectionRankCards.length} cards', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG - Collection card IDs: $collectionCardIds', isOn: LOGGING_SWITCH);
    
    // Filter out collection rank cards
    final playableCards = availableCards.where((cardId) => !collectionCardIds.contains(cardId)).toList();
    _logger.info('Cleco: DEBUG - Playable cards (after filtering collection): ${playableCards.length}', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Known card IDs: $knownCardIds', isOn: LOGGING_SWITCH);
    
    // Get unknown cards
    final unknownCards = playableCards.where((cardId) => !knownCardIds.contains(cardId)).toList();
    _logger.info('Cleco: DEBUG - Unknown cards: ${unknownCards.length}', isOn: LOGGING_SWITCH);
    
    // Get known playable cards
    final knownPlayableCards = playableCards.where((cardId) => knownCardIds.contains(cardId)).toList();
    _logger.info('Cleco: DEBUG - Known playable cards: ${knownPlayableCards.length}', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - After null filtering - Available: ${availableCards.length}, Playable: ${playableCards.length}, Unknown: ${unknownCards.length}, Known: ${knownPlayableCards.length}', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - All cards data: ${allCardsData.length} cards', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Prepared game data with keys: ${result.keys.join(', ')}', isOn: LOGGING_SWITCH);
  ...
  String _selectCardLegacy(List<String> availableCards, Map<String, dynamic> cardSelection, Map<String, double> evaluationWeights, Map<String, dynamic> gameState) {
    ...
    final currentPlayer = gameState['currentPlayer'] as Map<String, dynamic>?;
    if (currentPlayer == null) {
      _logger.error('Cleco: No current player found in game state', isOn: LOGGING_SWITCH);
        return availableCards[_random.nextInt(availableCards.length)];
    }
    ...
    _logger.info('Cleco: DEBUG - Available cards: ${availableCards.length}, Playable cards: ${playableCards.length}, Collection cards: ${collectionCardIds.length}', isOn: LOGGING_SWITCH);
    
    if (playableCards.isEmpty) {
      _logger.warning('Cleco: All available cards are collection rank cards, using fallback', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Unknown cards: ${unknownCards.length}, Known playable cards: ${knownPlayableCards.length}', isOn: LOGGING_SWITCH);
    
    // Determine if we should play optimally based on difficulty
    final optimalPlayProb = _getOptimalPlayProbability(strategy);
    final shouldPlayOptimal = _random.nextDouble() < optimalPlayProb;
    
    _logger.info('Cleco: DEBUG - Strategy: $strategy, Optimal prob: $optimalPlayProb, Should play optimal: $shouldPlayOptimal', isOn: LOGGING_SWITCH);
    ...
    if (shouldPlayOptimal) {
      ...
      if (unknownCards.isNotEmpty) {
        ...
        _logger.info('Cleco: DEBUG - Selected unknown card: $selectedCard', isOn: LOGGING_SWITCH);
        return selectedCard;
      }
      ...
      if (knownPlayableCards.isNotEmpty) {
        ...
        _logger.info('Cleco: DEBUG - Selected highest points card: $selectedCard', isOn: LOGGING_SWITCH);
        return selectedCard;
      }
    }
    ...
    _logger.info('Cleco: DEBUG - Selected random fallback card: $selectedCard', isOn: LOGGING_SWITCH);
    return selectedCard;
  }
  ...
  String _selectHighestPointsCard(List<String> cardIds, Map<String, dynamic> gameState) {
    _logger.info('Cleco: DEBUG - _selectHighestPointsCard called with ${cardIds.length} card IDs', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Found ${allCards.length} total cards in game state', isOn: LOGGING_SWITCH);
    
    // Filter to only the cards we're considering
    final candidateCards = allCards.where((card) => cardIds.contains(card['id'])).toList();
    
    _logger.info('Cleco: DEBUG - Found ${candidateCards.length} candidate cards', isOn: LOGGING_SWITCH);
    
    if (candidateCards.isEmpty) {
      _logger.warning('Cleco: No candidate cards found, using random fallback', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Found ${nonJackCards.length} non-Jack cards', isOn: LOGGING_SWITCH);
    
    if (nonJackCards.isEmpty) {
      // If all are Jacks, return random
      _logger.warning('Cleco: All candidate cards are Jacks, using random fallback', isOn: LOGGING_SWITCH);
      return cardIds[_random.nextInt(cardIds.length)];
    }
    ...
    _logger.info('Cleco: DEBUG - Selected highest points card: $selectedCard (points: $highestPoints)', isOn: LOGGING_SWITCH);
    return selectedCard;
  }
  ...
  Map<String, dynamic> _prepareSameRankGameData(List<String> availableCards, Map<String, dynamic> currentPlayer, Map<String, dynamic> gameState) {
    ...
    _logger.info('Cleco: DEBUG - After null filtering same rank - Available: ${availableCards.length}, Known: ${knownSameRankCards.length}, Unknown: ${unknownSameRankCards.length}', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _prepareSpecialPlayGameData(
    ...
    _logger.info('Cleco: DEBUG - _prepareSpecialPlayGameData called for player $actingPlayerId, difficulty: $difficulty', isOn: LOGGING_SWITCH);
    ...
    if (actingPlayer == null || actingPlayer.isEmpty) {
      _logger.warning('Cleco: DEBUG - Acting player $actingPlayerId not found', isOn: LOGGING_SWITCH);
      return {
        'difficulty': difficulty,
    ...
    _logger.info('Cleco: DEBUG - Acting player known_cards structure: ${knownCards.keys.toList()}', isOn: LOGGING_SWITCH);
    
    final actingPlayerKnownCardsRaw = knownCards[actingPlayerId] as Map<String, dynamic>?;
    
    if (actingPlayerKnownCardsRaw != null) {
      _logger.info('Cleco: DEBUG - Found known_cards entry for acting player $actingPlayerId with ${actingPlayerKnownCardsRaw.length} cards', isOn: LOGGING_SWITCH);
      for (final entry in actingPlayerKnownCardsRaw.entries) {
        ...
        if (cardId.isNotEmpty && cardId != 'null') {
          if (entry.value is Map<String, dynamic>) {
            ...
            _logger.info('Cleco: DEBUG - Added known card: $cardId', isOn: LOGGING_SWITCH);
          } else {
            _logger.warning('Cleco: DEBUG - Known card entry value is not a Map: ${entry.value.runtimeType}', isOn: LOGGING_SWITCH);
          }
        }
      }
    } else {
      _logger.warning('Cleco: DEBUG - No known_cards entry found for acting player $actingPlayerId in known_cards structure', isOn: LOGGING_SWITCH);
      _logger.info('Cleco: DEBUG - Known_cards structure keys: ${knownCards.keys.toList()}', isOn: LOGGING_SWITCH);
    }
    ...
    _logger.info('Cleco: DEBUG - Prepared special play game data:', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG -   Acting player hand: ${actingPlayerHand.length} cards', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG -   Acting player known cards: ${actingPlayerKnownCards.length} cards', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG -   Acting player collection cards: ${actingPlayerCollectionCards.length} cards', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG -   All players data: ${allPlayersData.length} players', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _evaluateSpecialPlayRules(
    ...
    _logger.info('Cleco: DEBUG - _evaluateSpecialPlayRules called with ${strategyRules.length} rules, shouldPlayOptimal: $shouldPlayOptimal, eventName: $eventName', isOn: LOGGING_SWITCH);
    
    if (strategyRules.isEmpty) {
      _logger.info('Cleco: DEBUG - No strategy rules defined, returning use: false', isOn: LOGGING_SWITCH);
      return {
        'use': false,
    ...
    _logger.info('Cleco: DEBUG - Sorted rules by priority: ${sortedRules.map((r) => '${r['name']} (${r['priority']})').join(', ')}', isOn: LOGGING_SWITCH);
    ...
    for (final rule in sortedRules) {
      ...
      _logger.info('Cleco: DEBUG - Evaluating rule: $ruleName', isOn: LOGGING_SWITCH);
      ...
      if (condition != null) {
        ...
        _logger.info('Cleco: DEBUG - Rule $ruleName condition result: $conditionResult', isOn: LOGGING_SWITCH);
        ...
        if (conditionResult) {
          ...
          if (executionProb != null) {
            ...
            _logger.info('Cleco: DEBUG - Rule $ruleName execution probability for $difficulty: $prob, shouldExecute: $shouldExecute', isOn: LOGGING_SWITCH);
            
            if (!shouldExecute) {
              _logger.info('Cleco: DEBUG - Rule $ruleName skipped due to execution probability', isOn: LOGGING_SWITCH);
          ...
          if (action != null) {
            _logger.info('Cleco: DEBUG - Rule $ruleName condition passed, executing action', isOn: LOGGING_SWITCH);
            ...
            if (_isValidSpecialPlayResult(result, eventName)) {
              ...
              _logger.info('Cleco: DEBUG - Rule $ruleName returned invalid result (no valid targets), trying next rule', isOn: LOGGING_SWITCH);
        ...
        final action = rule['action'] as Map<String, dynamic>?;
        if (action != null) {
          _logger.info('Cleco: DEBUG - Rule $ruleName has no condition, executing action', isOn: LOGGING_SWITCH);
          final result = _executeSpecialPlayAction(action, gameData, eventName, ruleName);
    ...
    _logger.info('Cleco: DEBUG - No rules matched, returning use: false', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _executeSpecialPlayAction(
    ...
    _logger.info('Cleco: DEBUG - _executeSpecialPlayAction called with actionType: $actionType, eventName: $eventName, ruleName: $ruleName', isOn: LOGGING_SWITCH);
    ...
    switch (actionType) {
      case 'use_special_play':
        if (eventName == 'jack_swap') {
          ...
          _logger.info('Cleco: DEBUG - Executing jack_swap with target strategy: $targetStrategy', isOn: LOGGING_SWITCH);
          
          // Select targets based on strategy
          final targets = _selectJackSwapTargets(gameData, targetStrategy);
          
          _logger.info('Cleco: DEBUG - Target selection result: $targets', isOn: LOGGING_SWITCH);
          ...
          _logger.info('Cleco: DEBUG - Executing queen_peek with target strategy: $targetStrategy', isOn: LOGGING_SWITCH);
      ...
      case 'collect_from_discard':
        ...
        if (topCard == null) {
          _logger.warning('Cleco: DEBUG - No discard pile top card found, skipping collection', isOn: LOGGING_SWITCH);
          return {
            'use': false,
            'reasoning': ruleName,
          };
        ...
        if (topCardRank.toLowerCase() != actingPlayerCollectionRank.toLowerCase()) {
          _logger.info('Cleco: DEBUG - Top card rank $topCardRank does not match collection rank $actingPlayerCollectionRank', isOn: LOGGING_SWITCH);
          return {
            'use': false,
            'reasoning': ruleName,
          };
        ...
        if (ruleName == 'collect_if_completes_set' && actingPlayerCollectionCards.length == 3) {
          _logger.info('Cleco: DEBUG - Player has 3 collection cards, collecting 4th card to complete set', isOn: LOGGING_SWITCH);
          return {
            'use': true,
            'reasoning': ruleName,
          };
        ...
        if (ruleName == 'collect_if_same_rank') {
          _logger.info('Cleco: DEBUG - Top card rank matches collection rank, collecting it', isOn: LOGGING_SWITCH);
          return {
            'use': true,
            'reasoning': ruleName,
          };
  ...
  Map<String, dynamic> _selectJackSwapTargets(
    ...
    _logger.info('Cleco: DEBUG - _selectJackSwapTargets called with strategy: $targetStrategy', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _selectLowestOpponentHigherOwn(
    ...
    _logger.info('Cleco: DEBUG - Selecting lowest opponent card and higher own card', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Searching for highest point card in acting player\'s known cards (${actingPlayerKnownCards.length} cards)', isOn: LOGGING_SWITCH);
    ...
    if (highestOwnCard == null) {
      _logger.warning('Cleco: DEBUG - No known cards for acting player, using fallback', isOn: LOGGING_SWITCH);
      return _selectRandomTwoPlayers(actingPlayerId, actingPlayer, allPlayers, gameState);
    }
    
    _logger.info('Cleco: DEBUG - Found highest own card: ${highestOwnCard['cardId']} (${highestOwnPoints} points)', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Searching for lowest point card from other players\' known cards (${allPlayers.length - 1} other players)', isOn: LOGGING_SWITCH);
    ...
    if (lowestOpponentCard != null && lowestOpponentPlayerId != null) {
      _logger.info('Cleco: DEBUG - Found lowest opponent card: ${lowestOpponentCard['cardId']} (${lowestOpponentPoints} points) from player $lowestOpponentPlayerId', isOn: LOGGING_SWITCH);
      
      if (lowestOpponentPoints < highestOwnPoints) {
        _logger.info('Cleco: DEBUG - Beneficial swap found: own card (${highestOwnCard['cardId']}, ${highestOwnPoints} pts) <-> opponent card (${lowestOpponentCard['cardId']}, ${lowestOpponentPoints} pts) from player $lowestOpponentPlayerId', isOn: LOGGING_SWITCH);
        ...
        _logger.info('Cleco: DEBUG - Opponent card (${lowestOpponentPoints} pts) is not lower than own card (${highestOwnPoints} pts), not beneficial', isOn: LOGGING_SWITCH);
      }
    } else {
      _logger.info('Cleco: DEBUG - No opponent cards found in known cards', isOn: LOGGING_SWITCH);
    }
    
    _logger.info('Cleco: DEBUG - No beneficial swap found, using fallback', isOn: LOGGING_SWITCH);
    return _selectRandomTwoPlayers(actingPlayerId, actingPlayer, allPlayers, gameState);
  }
  ...
  Map<String, dynamic> _selectRandomTwoPlayers(
    ...
    _logger.info('Cleco: DEBUG - Selecting random 2 cards from 2 other players', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Acting player has ${collectionCardIds.length} collection cards to exclude', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Acting player hand: ${actingPlayerHand.length} total, ${playableOwnCards.length} playable (excluding collection)', isOn: LOGGING_SWITCH);
    
    if (playableOwnCards.isEmpty) {
      _logger.warning('Cleco: DEBUG - No playable cards for acting player, using fallback', isOn: LOGGING_SWITCH);
      return {
        'first_card_id': null,
        'first_player_id': actingPlayerId,
        'second_card_id': null,
    ...
    if (otherPlayers.length < 2) {
      _logger.warning('Cleco: DEBUG - Not enough other players (need 2, have ${otherPlayers.length}), using single player', isOn: LOGGING_SWITCH);
    ...
    if (playableOwnCards.isEmpty || playableFirstOtherCards.isEmpty) {
      _logger.warning('Cleco: DEBUG - Not enough playable cards, using fallback', isOn: LOGGING_SWITCH);
      return {
        'first_card_id': null,
        'first_player_id': actingPlayerId,
        'second_card_id': null,
    ...
    _logger.info('Cleco: DEBUG - Selected random swap:', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG -   First card: $firstCard from acting player $actingPlayerId', isOn: LOGGING_SWITCH);
    _logger.info('Cleco: DEBUG -   Second card: $secondCard from other player $firstOtherPlayerId', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _selectQueenPeekTargets(
    ...
    _logger.info('Cleco: DEBUG - _selectQueenPeekTargets called with strategy: $targetStrategy', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _selectOwnUnknownCard(
    ...
    _logger.info('Cleco: DEBUG - Selecting own unknown card (excluding collection)', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Acting player hand: ${actingPlayerHand.length} total, ${playableHand.length} playable (excluding collection)', isOn: LOGGING_SWITCH);
    
    if (playableHand.isEmpty) {
      _logger.warning('Cleco: DEBUG - No playable cards in hand (all are collection cards), returning invalid result', isOn: LOGGING_SWITCH);
      return {
        'target_card_id': null,
    ...
    _logger.info('Cleco: DEBUG - Acting player has ${knownCardIds.length} known cards', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Found ${unknownCards.length} unknown cards in hand (out of ${playableHand.length} playable)', isOn: LOGGING_SWITCH);
    
    if (unknownCards.isEmpty) {
      _logger.warning('Cleco: DEBUG - All playable cards are already known, returning invalid result', isOn: LOGGING_SWITCH);
      return {
        'target_card_id': null,
        'target_player_id': null,
      };
    ...
    _logger.info('Cleco: DEBUG - Selected own unknown card: $selectedCardId', isOn: LOGGING_SWITCH);
  ...
  Map<String, dynamic> _selectRandomOtherPlayerCard(
    ...
    _logger.info('Cleco: DEBUG - Selecting random other player card (excluding collection)', isOn: LOGGING_SWITCH);
    ...
    if (allPlayerEntries.isEmpty) {
      _logger.warning('Cleco: DEBUG - No players found, returning invalid result', isOn: LOGGING_SWITCH);
      return {
        'target_card_id': null,
        'target_player_id': null,
      };
    ...
    for (final playerEntry in shuffledPlayers) {
      ...
      if (playerHand.isEmpty) {
        _logger.info('Cleco: DEBUG - Player $playerId has no cards, skipping', isOn: LOGGING_SWITCH);
        continue;
      }
      ...
      if (playableCards.isEmpty) {
        _logger.info('Cleco: DEBUG - Player $playerId has no playable cards (all are collection), skipping', isOn: LOGGING_SWITCH);
        continue;
      }
      ...
      _logger.info('Cleco: DEBUG - Selected random card: $selectedCardId from player $playerId', isOn: LOGGING_SWITCH);
    ...
    _logger.warning('Cleco: DEBUG - No players with playable cards found, returning invalid result', isOn: LOGGING_SWITCH);
    return {
      'target_card_id': null,
      'target_player_id': null,
    };
