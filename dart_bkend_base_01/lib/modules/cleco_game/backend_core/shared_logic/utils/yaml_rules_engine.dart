...
class YamlRulesEngine {
  ...
  String executeRules(List<dynamic> rules, Map<String, dynamic> gameData, bool shouldPlayOptimal) {
    _logger.info('Cleco: DEBUG - YamlRulesEngine.executeRules called with ${rules.length} rules, shouldPlayOptimal: $shouldPlayOptimal', isOn: LOGGING_SWITCH);
    ...
    if (!shouldPlayOptimal && sortedRules.isNotEmpty) {
      ...
      _logger.info('Cleco: DEBUG - Not playing optimally, using last rule: ${lastRule['name']}', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - No rules matched, using fallback logic', isOn: LOGGING_SWITCH);
    
    // Ultimate fallback: random from playable cards
    final playableCards = gameData['playable_cards'] as List<dynamic>? ?? [];
    if (playableCards.isNotEmpty) {
      _logger.info('Cleco: DEBUG - Using playable cards fallback: ${playableCards.length} cards', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Using available cards fallback: ${availableCards.length} cards', isOn: LOGGING_SWITCH);
  ...
  String _executeAction(Map<String, dynamic> action, Map<String, dynamic> gameData) {
    ...
    _logger.info('Cleco: DEBUG - _executeAction called with type: $type, source: $source, filters: ${filters.length}', isOn: LOGGING_SWITCH);
    
    // Get source data
    List<dynamic> sourceData = gameData[source] as List<dynamic>? ?? [];
    _logger.info('Cleco: DEBUG - Initial source data from $source: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    ...
    _logger.info('Cleco: DEBUG - Source data after null/collection filtering: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    
    // Apply filters
    for (final filter in filters) {
      if (filter is Map<String, dynamic>) {
        _logger.info('Cleco: DEBUG - Applying filter: ${filter['type']}', isOn: LOGGING_SWITCH);
        sourceData = _applyFilter(sourceData, filter, gameData);
        _logger.info('Cleco: DEBUG - After filter, source data: ${sourceData.length} items', isOn: LOGGING_SWITCH);
      }
    }
    ...
    if (sourceData.isEmpty) {
      ...
      _logger.info('Cleco: DEBUG - Source data empty, using playable cards fallback: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    ...
    if (sourceData.isEmpty) {
      ...
      _logger.info('Cleco: DEBUG - Still empty, using available cards fallback: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    ...
    if (sourceData.isEmpty) {
      _logger.warning('Cleco: DEBUG - All fallbacks failed, no cards available!', isOn: LOGGING_SWITCH);
    ...
    switch (type) {
      case 'select_random':
        ...
        _logger.info('Cleco: DEBUG - select_random result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_highest_points':
        result = _selectHighestPoints(sourceData, gameData);
        _logger.info('Cleco: DEBUG - select_highest_points result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_lowest_points':
        result = _selectLowestPoints(sourceData, gameData);
        _logger.info('Cleco: DEBUG - select_lowest_points result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_first':
        result = sourceData.first.toString();
        _logger.info('Cleco: DEBUG - select_first result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_last':
        result = sourceData.last.toString();
        _logger.info('Cleco: DEBUG - select_last result: $result', isOn: LOGGING_SWITCH);
        break;
      
      default:
        result = sourceData[_random.nextInt(sourceData.length)].toString();
        _logger.info('Cleco: DEBUG - default action result: $result', isOn: LOGGING_SWITCH);
        break;
    }
