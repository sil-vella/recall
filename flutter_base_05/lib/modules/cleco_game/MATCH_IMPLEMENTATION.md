## Cleco Match Screen – Flutter Implementation Plan

This document details the architecture and delivery plan for the Cleco match screen and related UI/logic, aligned with the Python backend `cleco_*` WebSocket events and the existing Flutter Base 05 patterns.

### Goals
- Build the match (gameplay) screen UI and client-side logic on top of existing managers (`ClecoGameManager`, `ClecoStateManager`, `StateManager`, `WebSocketManager`).
- Align with backend event names and payloads (`cleco_*`) for clean, testable integration.
...
- Route: `/cleco/game-play` (registered in `ClecoGameCore._registerScreens()` once ready).
- Base class: extend `BaseScreen` → `ClecoMatchScreen` and `_ClecoMatchScreenState`.
...
- Action bar: context-driven (Draw, Take from Discard, Play, Replace, Call Cleco, Use Power).
...
Managers:
- `ClecoGameManager`: orchestrates high-level flows; already listens to WS and updates `ClecoStateManager`.
- `ClecoStateManager`: holds current `GameState`; exposes streams for turn/hand/phase.
- `StateManager`: global store to render `MessageBoardWidget` and lobby info.

Event Registration (receive):
- Subscribe via `WebSocketManager`/`WSEventManager` to `cleco_*` events:
  - `cleco_game_state_updated` → update full `GameState` in `ClecoStateManager`.
  - `cleco_turn_changed` (or part of `game_state_updated`) → update current player.
  - `cleco_card_played`, `cleco_card_drawn` → per-action UI updates/animations.
  - `cleco_special_power_used` → trigger power dialogs result handling.
  - `cleco_cleco_called` → enter cleco phase, show banners/timers.
  - `cleco_game_ended` → show results dialog/panel.
  - `cleco_error` → toast/snackbar + session message board entry.

Event Emission (send):
- `cleco_join_game`, `cleco_leave_game` managed by lobby → reused if needed.
- `cleco_player_action` payloads:
...
Opponents Panel:
- Show each opponent’s name, card count, last action icon, cleco-called badge.
...
- Buttons enabled by rule state and `ClecoStateManager`:
  - Draw (face-down), Take from Discard
  - Play, Replace, Place Drawn & Play, Place Drawn & Replace
  - Call Cleco, Use Power (with dialog)
- Semantics: `match_action_draw`, `match_action_take_discard`, `match_action_play`, `match_action_replace`, `match_action_call_cleco`, `match_action_use_power`.
...
From `ClecoStateManager` derive:
- `isMyTurn`, `canCallCleco`, `phase`, `turnNumber`, `roundNumber`, `currentPlayerId`.
- Current hand, playable cards (backend determines playable cards, frontend only displays).
- Disable UI actions when not permitted; attempt anyway → server returns `cleco_error` to show message.
...
- On `cleco_error`: show snackbar + add session message via `ClecoMessageManager`.
...
- Integration (manual + CDP console): semantics-driven end-to-end script (open drawer, navigate, join, draw, play, cleco).
...
- Add `/cleco/game-play` screen with header/center/hand skeleton.
- Wire event listeners (`cleco_*`) and update `ClecoStateManager` mappings.
...
Phase D – Cleco and Results
- Implement Call Cleco flow; final round state; results panel.
...
- Confirm payload contracts for:
  - `cleco_game_state_updated`, `cleco_card_played`, `cleco_card_drawn`, `cleco_turn_changed`, `cleco_cleco_called`, `cleco_special_power_used`, `cleco_game_ended`, `cleco_error`.
- Confirm action payloads accepted under `cleco_player_action` for all listed actions.
