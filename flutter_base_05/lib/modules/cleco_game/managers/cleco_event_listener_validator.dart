...
class ClecoGameEventListenerValidator {
  ...
  static ClecoGameEventListenerValidator get instance {
    _instance ??= ClecoGameEventListenerValidator._internal();
    return _instance!;
  }
  
  final Logger _logger = Logger();
  
  ClecoGameEventListenerValidator._internal();
  ...
  static const Map<String, EventConfig> _eventConfigs = {
    ...
    'cleco_new_player_joined': EventConfig(
      schema: {'event_type', 'room_id', 'owner_id', 'joined_player', 'game_state', 'timestamp'},
      handlerMethod: 'handleClecoNewPlayerJoined',
    ),
    'cleco_joined_games': EventConfig(
      schema: {'event_type', 'user_id', 'session_id', 'games', 'total_games', 'timestamp'},
      handlerMethod: 'handleClecoJoinedGames',
    ),
    ...
    'cleco_error': EventConfig(
      schema: {'message'},
      handlerMethod: 'handleClecoError',
    ),
  ...
  /// Handle direct game events and route to ClecoEventManager
  void _handleDirectEvent(String eventType, Map<String, dynamic> data) {
    try {
      ...
      // Route directly to ClecoEventManager based on event type
  ...
  /// Route validated events directly to ClecoEventManager methods
  void _routeEventToManager(String eventType, Map<String, dynamic> eventPayload) {
    ...
    try {
      switch (handlerMethod) {
        case 'handleClecoNewPlayerJoined':
          ClecoEventHandlerCallbacks.handleClecoNewPlayerJoined(eventPayload);
          break;
        case 'handleClecoJoinedGames':
          ClecoEventHandlerCallbacks.handleClecoJoinedGames(eventPayload);
          break;
        case 'handleGameStarted':
          ClecoEventHandlerCallbacks.handleGameStarted(eventPayload);
          break;
        case 'handleTurnStarted':
          ClecoEventHandlerCallbacks.handleTurnStarted(eventPayload);
          break;
        case 'handleGameStateUpdated':
          ClecoEventHandlerCallbacks.handleGameStateUpdated(eventPayload);
          break;
        case 'handleGameStatePartialUpdate':
          ClecoEventHandlerCallbacks.handleGameStatePartialUpdate(eventPayload);
          break;
        case 'handlePlayerStateUpdated':
          ClecoEventHandlerCallbacks.handlePlayerStateUpdated(eventPayload);
          break;
        case 'handleQueenPeekResult':
          ClecoEventHandlerCallbacks.handleQueenPeekResult(eventPayload);
          break;
        case 'handleClecoError':
          ClecoEventHandlerCallbacks.handleClecoError(eventPayload);
          break;
        default:
          return;
      }
