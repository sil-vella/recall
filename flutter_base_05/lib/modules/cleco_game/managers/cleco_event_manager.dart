...
import '../../cleco_game/managers/cleco_event_listener_validator.dart';
...
class ClecoEventManager {
  ...
  Future<bool> initialize() async {
    try {
      ...
      _stateManager.registerModuleState("cleco_messages", {
      ...
      // Register cleco-specific event listeners
      _registerClecoEventListeners();

      // Cleco-specific Socket.IO listeners are centralized in ClecoGameCoordinator.
  ...
  void _registerClecoEventListeners() {
    // Initialize the event listener validator
    ClecoGameEventListenerValidator.instance.initialize();
  }
  ...
  void handleClecoNewPlayerJoined(Map<String, dynamic> data) {
    ClecoEventHandlerCallbacks.handleClecoNewPlayerJoined(data);
  }

  /// Handle cleco_joined_games event
  void handleClecoJoinedGames(Map<String, dynamic> data) {
    ClecoEventHandlerCallbacks.handleClecoJoinedGames(data);
  }

  /// Handle game_started event
  void handleGameStarted(Map<String, dynamic> data) {
    ClecoEventHandlerCallbacks.handleGameStarted(data);
  }

  /// Handle turn_started event
  void handleTurnStarted(Map<String, dynamic> data) {
    ClecoEventHandlerCallbacks.handleTurnStarted(data);
  }
  ...
  void handleGameStatePartialUpdate(Map<String, dynamic> data) {
    ...
    ClecoEventHandlerCallbacks.handleGameStatePartialUpdate(data);
  }

  /// Handle player_state_updated event
  void handlePlayerStateUpdated(Map<String, dynamic> data) {
    ClecoEventHandlerCallbacks.handlePlayerStateUpdated(data);
  }
  ...
  void _registerHookCallbacks() {
    ...
    HooksManager().registerHookWithData('websocket_connect', (data) {
      ...
      if (status == 'connected') {
        // Update cleco game connection status
        ClecoGameHelpers.updateConnectionStatus(isConnected: true);
    ...
    HooksManager().registerHookWithData('websocket_disconnect', (data) {
      ...
      if (status == 'disconnected') {
        // Update cleco game connection status
        ClecoGameHelpers.updateConnectionStatus(isConnected: false);
    ...
    HooksManager().registerHookWithData('websocket_join_room', (data) {
      try {
        ...
        final clecoState = StateManager().getModuleState<Map<String, dynamic>>('cleco_game') ?? {};
        final isRandomJoinInProgress = clecoState['isRandomJoinInProgress'] == true;
        
        _logger.info('🔍 websocket_join_room: isRandomJoinInProgress=$isRandomJoinInProgress, clecoState keys: ${clecoState.keys.toList()}', isOn: LOGGING_SWITCH);
        ...
        if (status == 'success' && isRandomJoinInProgress && roomId.isNotEmpty) {
          ...
          // Use a small delay to ensure state is fully updated
          Future.delayed(const Duration(milliseconds: 300), () {
            _logger.info('🎮 Executing navigation to /cleco/game-play', isOn: LOGGING_SWITCH);
            NavigationManager().navigateTo('/cleco/game-play');
          });
        } else {
    ...
    HooksManager().registerHookWithData('websocket_user_joined_rooms', (data) {
      ...
      final totalRooms = data['total_rooms'] ?? 0;
              ...
              // Update cleco game state to reflect the current room membership
        ...
        // This will be handled by the cleco_joined_games event when it's sent
      }
    });
