...
class PlayerAction {
  static final ClecoGameEventEmitter _eventEmitter = ClecoGameEventEmitter.instance;
  ...
  /// Check if the current game is a cleco game
  bool _checkIfPracticeGame() {
    try {
      // Use the centralized game state accessor
      final gameAccessor = ClecoGameStateAccessor.instance;
  ...
  /// Trigger cleco event by calling the PracticeGameCoordinator directly
  void _triggerPracticeEvent() {
    try {
      // Get the cleco game coordinator (singleton)
      // todo ... link to practice mode logic (dart bkend replica)
      
    } catch (e) {
      _logger.error('Error triggering cleco event: $e', isOn: LOGGING_SWITCH);
    }
  }
  ...
  static PlayerAction playerDraw({
    ...
    // playerId is now auto-added by ClecoGameEventEmitter
  }) {
  ...
  static PlayerAction getPublicRooms() {
    return PlayerAction._(
      ...
      eventName: 'cleco_get_public_rooms',
      payload: {},
    );
  }
