...
```dart
// 1. Get the cleco_game module state
...
final currentGameId = gameInfo['currentGameId']?.toString() ?? ''; // OR clecoGameState['currentGameId']
...
- [ ] Verify `currentGameId` is set in both `clecoGameState['currentGameId']` and `gameInfo['currentGameId']`
