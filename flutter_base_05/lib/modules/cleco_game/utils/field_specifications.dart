...
/// This file defines the validation rules for all cleco game data

/// Exception thrown when event validation fails
class ClecoEventException implements Exception {
  final String message;
  final String? eventType;
  final String? fieldName;
  
  ClecoEventException(this.message, {this.eventType, this.fieldName});
  
  @override
  String toString() {
    if (eventType != null && fieldName != null) {
      return 'ClecoEventException in $eventType.$fieldName: $message';
    } else if (eventType != null) {
      return 'ClecoEventException in $eventType: $message';
    }
    return 'ClecoEventException: $message';
  }
}

/// Exception thrown when state validation fails
class ClecoStateException implements Exception {
  final String message;
  final String? fieldName;
  
  ClecoStateException(this.message, {this.fieldName});
  
  @override
  String toString() {
    if (fieldName != null) {
      return 'ClecoStateException in $fieldName: $message';
    }
    return 'ClecoStateException: $message';
  }
}

/// Specification for validating event fields
class ClecoEventFieldSpec {
  final Type type;
  final bool required;
