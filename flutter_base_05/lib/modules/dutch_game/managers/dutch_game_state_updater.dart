...
class DutchGameStateUpdater {
  ...
  static const bool LOGGING_SWITCH = true; // Enabled for action interception and snapshot testing
  ...
  void _applyValidatedUpdates(Map<String, dynamic> validatedUpdates) {
    ...
    try {
      ...
      // INTERCEPT ACTIONS: Extract actions from validated updates before state merging
      // Pass deep copy of current state so snapshot can use old state
      final currentStateDeepCopy = _deepCopyState(currentState);
      _interceptActions(validatedUpdates, currentState, currentStateDeepCopy);
  ...
  /// Deep copy state for snapshot (to avoid reference issues)
  Map<String, dynamic> _deepCopyState(Map<String, dynamic> state) {
    try {
      // Use JSON encode/decode for deep copy
      final jsonString = jsonEncode(state);
      final copiedState = jsonDecode(jsonString) as Map<String, dynamic>;
      return _deepConvertToMapStringDynamic(copiedState) as Map<String, dynamic>;
    } catch (e) {
      if (LOGGING_SWITCH) {
        _logger.warning('🎬 ACTION_INTERCEPT: Failed to deep copy state: $e');
      }
  ...
  /// Intercept actions from validated updates before state merging
  /// Extracts action and actionData from players and triggers snapshot capture
  /// Passes old state deep copy so snapshot widget can render with old state
  void _interceptActions(Map<String, dynamic> validatedUpdates, Map<String, dynamic> currentState, Map<String, dynamic> oldStateDeepCopy) {
    if (LOGGING_SWITCH) {
      _logger.info('🎬 ACTION_INTERCEPT: Starting action interception check');
      _logger.debug('🎬 ACTION_INTERCEPT: Validated updates keys: ${validatedUpdates.keys.toList()}');
    }
    
    try {
      // Check if games map is being updated
      final gamesUpdate = validatedUpdates['games'] as Map<String, dynamic>?;
      if (gamesUpdate == null) {
        if (LOGGING_SWITCH) {
          _logger.debug('🎬 ACTION_INTERCEPT: No games update found in validated updates');
        }
        return;
      }
      
      if (LOGGING_SWITCH) {
        _logger.debug('🎬 ACTION_INTERCEPT: Found games update with ${gamesUpdate.length} game(s)');
      }
      
      // Iterate through all games in the update
      for (final gameEntry in gamesUpdate.entries) {
        final gameId = gameEntry.key;
        final gameUpdate = gameEntry.value as Map<String, dynamic>?;
        if (gameUpdate == null) {
          if (LOGGING_SWITCH) {
            _logger.debug('🎬 ACTION_INTERCEPT: Skipping game $gameId - gameUpdate is null');
          }
          continue;
        }
        
        final gameDataUpdate = gameUpdate['gameData'] as Map<String, dynamic>?;
        if (gameDataUpdate == null) {
          if (LOGGING_SWITCH) {
            _logger.debug('🎬 ACTION_INTERCEPT: Skipping game $gameId - gameDataUpdate is null');
          }
          continue;
        }
        
        final gameStateUpdate = gameDataUpdate['game_state'] as Map<String, dynamic>?;
        if (gameStateUpdate == null) {
          if (LOGGING_SWITCH) {
            _logger.debug('🎬 ACTION_INTERCEPT: Skipping game $gameId - gameStateUpdate is null');
          }
          continue;
        }
        
        final playersUpdate = gameStateUpdate['players'] as List<dynamic>?;
        if (playersUpdate == null) {
          if (LOGGING_SWITCH) {
            _logger.debug('🎬 ACTION_INTERCEPT: Skipping game $gameId - playersUpdate is null');
          }
          continue;
        }
        
        if (LOGGING_SWITCH) {
          _logger.debug('🎬 ACTION_INTERCEPT: Checking ${playersUpdate.length} player(s) in game $gameId for actions');
        }
        
        // Check each player for actions
        for (int i = 0; i <playersUpdate.length; i++) {
          final player = playersUpdate[i];
          if (player is! Map<String, dynamic>) {
            if (LOGGING_SWITCH) {
              _logger.debug('🎬 ACTION_INTERCEPT: Player at index $i is not a Map, skipping');
            }
            continue;
          }
          ...
          if (LOGGING_SWITCH) {
            _logger.debug('🎬 ACTION_INTERCEPT: Checking player $playerId (index $i) - action: $action, hasActionData: ${actionData != null}');
          }
          
          if (action != null && action.isNotEmpty && actionData != null) {
            if (LOGGING_SWITCH) {
              _logger.info('🎬 ACTION_INTERCEPT: ✅ ACTION DETECTED!');
              _logger.info('🎬 ACTION_INTERCEPT: Action type: "$action"');
              _logger.info('🎬 ACTION_INTERCEPT: Player ID: $playerId');
              _logger.info('🎬 ACTION_INTERCEPT: Game ID: $gameId');
              _logger.info('🎬 ACTION_INTERCEPT: ActionData: $actionData');
              
              // Log actionData details
              if (actionData.containsKey('card1Data')) {
                final card1Data = actionData['card1Data'] as Map<String, dynamic>?;
                if (card1Data != null) {
                  _logger.info('🎬 ACTION_INTERCEPT: card1Data - cardId: ${card1Data['cardId']}, playerId: ${card1Data['playerId']}');
                }
              }
              if (actionData.containsKey('card2Data')) {
                final card2Data = actionData['card2Data'] as Map<String, dynamic>?;
                if (card2Data != null) {
                  _logger.info('🎬 ACTION_INTERCEPT: card2Data - cardId: ${card2Data['cardId']}, playerId: ${card2Data['playerId']}');
                }
              }
            }
            
            // Trigger snapshot capture by setting flag in state synchronously
            // This will be picked up by UnifiedGameBoardWidget BEFORE state merge
            // Pass old state deep copy so snapshot widget can render with old state
            // Use updateStateSync to set the trigger immediately without queuing
            final timestamp = DateTime.now().millisecondsSinceEpoch;
            if (LOGGING_SWITCH) {
              _logger.info('🎬 ACTION_INTERCEPT: Triggering snapshot capture with timestamp: $timestamp');
              _logger.debug('🎬 ACTION_INTERCEPT: Passing old state deep copy (${oldStateDeepCopy.keys.length} keys)');
            }
            ...
            updateStateSync({
              'snapshotTrigger': {
                ...
                'gameId': gameId,
                'timestamp': timestamp,
                'oldState': oldStateDeepCopy, // Deep copy of state before update
              },
            });
            
            if (LOGGING_SWITCH) {
              _logger.info('🎬 ACTION_INTERCEPT: Snapshot trigger set successfully - UnifiedGameBoardWidget should capture snapshot now');
            }
            
            // Only trigger once per action (break after first action found)
            return;
          }
        }
      }
      ...
      if (LOGGING_SWITCH) {
        _logger.debug('🎬 ACTION_INTERCEPT: No actions found in validated updates');
      }
    } catch (e, stackTrace) {
      if (LOGGING_SWITCH) {
        _logger.error('🎬 ACTION_INTERCEPT: ❌ ERROR intercepting actions: $e', error: e, stackTrace: stackTrace);
      }
    }
  }
