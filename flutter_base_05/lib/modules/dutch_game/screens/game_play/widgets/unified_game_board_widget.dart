...
Map<String, dynamic> _unifiedBoardViewSlice(Map<String, dynamic> d) {
  ...
  return {
    ...
    'boardGameState': <String, dynamic>{
      ...
      'dutchActive': gs['dutchActive'] ?? false,
      'dutchCalledBy': gs['dutchCalledBy']?.toString(),
...
class _UnifiedGameBoardWidgetState extends State<UnifiedGameBoardWidget> with TickerProviderStateMixin {
  ...
  _OppBoardContext _prepareOppBoardContext(Map<String, dynamic> board) {
    ...
    final gamePhaseForPeek =
        (board['boardGameState'] as Map<String, dynamic>? ?? {})['phase']
            ?.toString();
    final demoInitialPeekPending = _demoInitialPeekSelectionPending();
    final peekRevealAllowed = demoInitialPeekPending ||
        DutchGameHelpers.statusAllowsPeekReveal(
          playerStatus,
          gamePhase: gamePhaseForPeek,
        );

    if (!peekRevealAllowed && _isCardsToPeekProtected && !demoInitialPeekPending) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    if (peekRevealAllowed &&
        (playerStatus == 'initial_peek' || demoInitialPeekPending) &&
  ...
  Widget _paddedOpponentSlot(
    ...
    final dutchActive = bgs['dutchActive'] as bool? ?? false;
    final dutchCalledBy = bgs['dutchCalledBy']?.toString();
  ...
  Widget _buildOpponentCard(Map<String, dynamic> player, List<dynamic> cardsToPeek, List<dynamic> playerCollectionRankCards, bool isCurrentTurn, bool isGameActive, bool isCurrentPlayer, String currentPlayerStatus, Map<String, dynamic>? knownCards, bool isInitialPeekPhase, String? phase, Map<String, int>? timerConfig, {required bool dutchActive, String? dutchCalledBy, required CardTableOrientation cardTableOrientation, bool centerVerticallyInSideColumn = false, bool isDealingVisual = false}) {
    ...
    final playerId = player['id']?.toString() ?? '';
    final isDutchCaller = _isPlayerDutchCaller(player, dutchCalledBy: dutchCalledBy);
    final inDutchPhase = dutchActive && isGameActive;
    final showDutchCallerHud = isDutchCaller && inDutchPhase;
    ...
    // Per-seat status from SSOT (not the local user's status).
    final shouldShowTimer = _seatStatusShowsActionTimer(playerStatus);
    ...
    // Highlight current turn OR this seat's own action (jack swap, peek, draw, etc.).
    // Same-rank window keeps everyone on same_rank_window — only currentPlayer gets turn ring.
  ...
  /// Orange ring/chip/border for the player who called Dutch (matches table Dutch glow).
  static const Color _dutchHighlightColor = AppColors.callDutchChipBackground;

  bool _isPlayerDutchCaller(
    Map<String, dynamic> player, {
    String? dutchCalledBy,
  }) {
    final playerId = player['id']?.toString() ?? '';
    if (player['hasCalledDutch'] == true) return true;
    final callerId = dutchCalledBy?.trim() ?? '';
    return callerId.isNotEmpty && playerId == callerId;
  }

  /// Avatar HUD ring: static orange when [isDutchCaller], else animated timer or plain avatar.
  Widget _buildHudAvatarRing({
    required bool isDutchCaller,
    required bool showAnimatedTimer,
    required int durationSeconds,
    required Color timerColor,
    required Widget centerChild,
    bool emphasize = false,
    String? timerKeySuffix,
  }) {
    final size = emphasize ? (_kHudRingOuter + 4) : _kHudRingOuter;
    final sw = emphasize ? (_kHudRingStroke + 0.5) : _kHudRingStroke;
    final inner = (size - 2 * sw - 2).clamp(0.0, double.infinity);
          ...
          children: [
            SizedBox(
              ...
              child: CircularProgressIndicator(
                ...
                valueColor: const AlwaysStoppedAnimation<Color>(_dutchHighlightColor),
              ),
            ),
            SizedBox(
              width: inner,
              height: inner,
              child: ClipOval(child: centerChild),
            ),
          ],
        ),
      );
    ...
    if (showAnimatedTimer) {
      return CircularTimerWidget(
        key: ValueKey('timer_${timerKeySuffix ?? 'hud'}'),
  ...
  /// True when this seat is in an action status that should show timer ring / emphasis.
  bool _seatStatusShowsActionTimer(String status) {
    switch (status) {
      case 'drawing_card':
      case 'playing_card':
      case 'initial_peek':
  ...
  /// True when this seat (not the whole table) is performing a special / turn action.
  bool _seatStatusShowsActionHighlight(String status) {
    switch (status) {
      case 'drawing_card':
      case 'playing_card':
      case 'initial_peek':
  ...
  /// Demo initial peek: ignore server phase timer until the user has picked 2 cards.
  bool _demoInitialPeekSelectionPending() {
    if (DemoActionHandler.getActiveDemoActionType() != 'initial_peek') {
      return false;
    }
    final dutch =
    ...
    final games = dutch['games'] as Map<String, dynamic>? ?? {};
    final gameState = games[gameId]?['gameData']?['game_state'] as Map<String, dynamic>?;
    if (gameState?['gameType']?.toString() != 'demo') return false;

    final peek = dutch['myCardsToPeek'] as List<dynamic>? ?? [];
    if (peek.length >= 2 && DutchGameHelpers.peekListHasFullData(peek)) {
      return false;
    }
    if (DemoFunctionality.instance.getInitialPeekSelectedCardIds().length >= 2) {
      return false;
    }
    return true;
  }
  ...
  Widget _buildGameBoard(Map<String, dynamic> board) {
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          // Draw pile, match pot, discard — vertically centered as one block in the middle column.
          final gameboardRowWidth = constraints.maxWidth;
          
          final boardHeight = constraints.maxHeight;
          return SizedBox(
            height: boardHeight.isFinite ? boardHeight : null,
  ...
  Widget _buildMyHand(Map<String, dynamic> board) {
    ...
    if (isCardsToPeekEmpty && _isMyHandCardsToPeekProtected && !demoInitialPeekPending) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    if (!peekRevealAllowed && _isMyHandCardsToPeekProtected && !demoInitialPeekPending) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    final effectiveStatusForHand =
        demoInitialPeekPending ? 'initial_peek' : playerStatusForHand;
    final effectivePhaseForHand =
        demoInitialPeekPending ? 'initial_peek' : gamePhaseForHand;
    final inInitialPeekUi = effectiveStatusForHand == 'initial_peek' ||
        (effectiveStatusForHand == 'waiting' &&
            effectivePhaseForHand == 'initial_peek');
    ...
    final dutchCalledBy = gameState['dutchCalledBy']?.toString();
    ...
    if (playerStatus != 'initial_peek' &&
        !_demoInitialPeekSelectionPending() &&
        _initialPeekSelectedCardIds.isNotEmpty) {
      _initialPeekSelectedCardIds.clear();
    }
    ...
    final isInInitialPeek = demoInitialPeekPending ||
        playerStatus == 'initial_peek' ||
        gamePhaseForMyHand == 'initial_peek';
    final isInSameRank =
        playerStatus == 'same_rank_window' ||
        gamePhaseForMyHand == 'same_rank_window';
    final showMyHandTimer = demoInitialPeekPending
        ? false
        : _myHandShowsAvatarTimer(
    ...
    final Widget? mySeatCenterHudOverlay = cards.isEmpty
        ...
        : Row(
            ...
            children: [
              _buildHudAvatarRing(
                isDutchCaller: showLocalDutchCallerHud,
                showAnimatedTimer: showMyHandTimer,
                ...
                emphasize: emphasizeMyHandAvatar,
                timerKeySuffix: 'myhand_$playerStatus',
              ),
            ],
          );
  ...
  Widget _buildMyHandCardsGrid(
    ...
    return LayoutBuilder(
          builder: (context, constraints) {
            ...
            final demoInitialPeekPending = _demoInitialPeekSelectionPending();
            final statusForHandGlow = demoInitialPeekPending
                ? 'initial_peek'
                : playerStatusForHand;
            final phaseForHandGlow = demoInitialPeekPending
                ? 'initial_peek'
                : gamePhaseForHand;
            final currentPlayerStatus = _myHandCardGlowStatus(
              statusForHandGlow,
              phaseForHandGlow,
        ...
        final currentPlayerStatusForGlow = _myHandCardGlowStatus(
          statusForHandGlow,
          phaseForHandGlow,
          revealedPeekCount: cardsToPeek.length,
        );
  ...
  Future<void> _handleCallDutch(BuildContext context, String gameId) async {
    ...
    try {
      ...
      final callDutchAction = PlayerAction.callDutch(gameId: gameId);
      
      await callDutchAction.execute();
      ...
      if (mounted) {
        setState(() {
          _isProcessingAction = false;
          _callDutchTappedPending = false; // Restore button on failure
        });
  ...
  void _handleMyHandCardSelection(BuildContext context, int index, Map<String, dynamic> card) async {
    ...
    final currentPlayerStatus = _getCurrentUserStatus();
    final demoInitialPeekPending = _demoInitialPeekSelectionPending();
    final effectivePlayerStatus =
        demoInitialPeekPending ? 'initial_peek' : currentPlayerStatus;
    
      
    if (effectivePlayerStatus == 'jack_swap') {
      
    }
    if (effectivePlayerStatus == 'playing_card' || 
        effectivePlayerStatus == 'jack_swap' || 
        effectivePlayerStatus == 'queen_peek' ||
        effectivePlayerStatus == 'same_rank_window' ||
        effectivePlayerStatus == 'initial_peek') {
      ...
      try {
        ...
        } else if (effectivePlayerStatus == 'jack_swap') {
          final currentUserId = DutchEventHandlerCallbacks.getCurrentUserId();
          try {
            await PlayerAction.selectCardForJackSwap(
              cardId: card['cardId']?.toString() ?? '',
        ...
        } else if (effectivePlayerStatus == 'queen_peek') {
          final currentUserId = DutchEventHandlerCallbacks.getCurrentUserId();
          final queenPeekAction = PlayerAction.queenPeek(
            gameId: currentGameId,
            cardId: card['cardId']?.toString() ?? '',
        ...
        } else if (effectivePlayerStatus == 'initial_peek') {
          final cardId = card['cardId']?.toString() ?? '';
          if (cardId.isEmpty) {
            return;
          }
