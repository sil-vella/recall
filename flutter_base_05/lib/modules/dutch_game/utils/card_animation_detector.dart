...
class CardAnimationDetector {
  ...
  /// Track recently queued actions to prevent duplicate queueing
  /// Key: '${playerId}_${action}_${actionDataHash}', Value: timestamp
  final Map<String, DateTime> _recentlyQueuedActions = {};
  
  /// Time window for deduplication (prevent same action from being queued within this duration)
  ...
  /// Detect player actions from game state and queue animations
  /// This method is called SYNCHRONOUSLY from the state updater (before widget recomputation)
  /// 
  /// [currentStateSlices] Previous state slices (OLD state - before updates)
  ///                      Contains: myHand, centerBoard, opponentsPanel from currentState
  /// [newState] Complete new state map (NEW state - after updates)
  ///            This is the merged state that will be used for widget recomputation
  /// 
  /// Flow:
  /// 1. Detect actions from newState
  /// 2. Capture currentStateSlices (OLD state) in CardAnimationManager for animation layer
  /// 3. Queue animations with action data
  /// 4. Clear actions from newState (to prevent re-queueing)
  /// 
  /// Returns true if an action was detected and processed, false otherwise
  bool detectAndQueueActionsFromState(
    Map<String, dynamic> currentStateSlices,
    Map<String, dynamic> newState,
  ) {
    try {
      // Detect action from newState (NEW state - after updates)
      final actionInfo = _detectPlayerActionsFromState(newState);
      if (actionInfo == null) {
        return false;
      }

      // Check for duplicate action (prevent re-queueing same action within deduplication window)
      final actionPlayerId = actionInfo['playerId'] as String;
      final action = actionInfo['action'] as String;
      final actionData = actionInfo['actionData'] as Map<String, dynamic>;
      final actionDataHash = actionData.toString().hashCode;
      final deduplicationKey = '${actionPlayerId}_${action}_$actionDataHash';
      
      final now = DateTime.now();
      final lastQueuedTime = _recentlyQueuedActions[deduplicationKey];
      ...
      // Record this action as queued
      _recentlyQueuedActions[deduplicationKey] = now;
      
      // Clean up old entries (older than deduplication window)
      _recentlyQueuedActions.removeWhere((key, timestamp) => 
      ...
      // We clear actions for ALL players (both human and CPU) to prevent re-detection
      // The backend manages the actual game logic, but the frontend needs to clear
      // the action flag to prevent the same action from being detected and queued
      // multiple times during rapid state updates
      // Note: Deduplication above also prevents duplicate queueing even if action persists
  ...
  Map<String, dynamic>? _detectPlayerActionsFromState(Map<String, dynamic> newState) {
    try {
  ...
  /// [state] The state map to modify (will be modified in place)
  /// [playerId] The player ID to clear action for
