...
class GameRound {
  ...
  void _checkPendingEventsBeforeEndingRound() {
    ...
    try {
      ...
      Logger().error('Error checking pending events: $e', isOn: loggingSwitch);
    }
  }
    ...
    try {
      ...
      switch (eventType) {
        ...
        case 'jack_swap_pause':
          ...
          break;
        default:
          Logger().info('Unknown pending event type: $eventType', isOn: loggingSwitch);
      }
    } catch (e) {
      Logger().error('Error processing pending event: $e', isOn: loggingSwitch);
    }
  }
  ...
  void _handleQueenPeekPause(Map<String, dynamic> data) {
    ...
    try {
      ...
      Logger().info('Queen peek pause for player $playerId, card $cardId', isOn: loggingSwitch);
      
      // In a real implementation, this would show the card to the player
      // and wait for them to acknowledge before continuing
      
    } catch (e) {
      Logger().error('Error handling queen peek pause: $e', isOn: loggingSwitch);
    }
  }
  ...
  void _handleJackSwapPause(Map<String, dynamic> data) {
    ...
    try {
      ...
      Logger().info('Jack swap pause for player $playerId, cards $card1Id and $card2Id', isOn: loggingSwitch);
      
      // In a real implementation, this would show the swapped cards
      // and wait for players to acknowledge before continuing
      
    } catch (e) {
      Logger().error('Error handling jack swap pause: $e', isOn: loggingSwitch);
    }
  }
  ...
  void _moveToNextPlayer() {
    ...
    try {
      ...
      if (playerIds.isEmpty) {
        Logger().error('No players in game', isOn: loggingSwitch);
        return;
      }
      ...
      Logger().info('Moved to next player: $nextPlayerId', isOn: loggingSwitch);
      
    } catch (e) {
      Logger().error('Error moving to next player: $e', isOn: loggingSwitch);
    }
  }
  ...
  void _sendTurnStartedEvent() {
    ...
    try {
      ...
      Logger().error('Error sending turn started event: $e', isOn: loggingSwitch);
    }
  }
  ...
  void _logAction(String action, Map<String, dynamic> data) {
    ...
    try {
      ...
      Logger().info('Action logged: $action', isOn: loggingSwitch);
      
    } catch (e) {
      Logger().error('Error logging action: $e', isOn: loggingSwitch);
    }
  }
  ...
  bool _handleQueenPeek(String userId, Map<String, dynamic> actionData) {
    ...
    try {
      Logger().info('Handling Queen peek for player $userId with data: $actionData', isOn: loggingSwitch);
      ...
      if (cardId == null || ownerId == null) {
        Logger().error('Missing required data for queen peek: card_id=$cardId, ownerId=$ownerId', isOn: loggingSwitch);
        return false;
      }
      ...
      if (targetPlayer == null) {
        Logger().error('Target player $ownerId not found for queen peek', isOn: loggingSwitch);
        return false;
      }
      ...
      if (targetCard == null) {
        Logger().error('Card $cardId not found in target player $ownerId hand', isOn: loggingSwitch);
        return false;
      }
      ...
      if (currentPlayer == null) {
        Logger().error('Current player $userId not found for queen peek', isOn: loggingSwitch);
        return false;
      }
      ...
      Logger().info('Added card $cardId to player $userId cards_to_peek list', isOn: loggingSwitch);
      
      // Set player status to PEEKING
      currentPlayer.updateStatus(PlayerStatus.peeking);
      Logger().info('Set player $userId status to PEEKING', isOn: loggingSwitch);
      
      return true;
      
    } catch (e) {
      Logger().error('Error in _handleQueenPeek: $e', isOn: loggingSwitch);
      return false;
    }
  }
  ...
  bool _handleJackSwap(String userId, Map<String, dynamic> actionData) {
    ...
    try {
      Logger().info('Handling Jack swap for player $userId with data: $actionData', isOn: loggingSwitch);
      ...
      if (firstCardId == null || firstPlayerId == null || secondCardId == null || secondPlayerId == null) {
        Logger().error('Invalid Jack swap data - missing required fields', isOn: loggingSwitch);
        return false;
      }
      
      // Validate both players exist
      if (!gameState.players.containsKey(firstPlayerId) || !gameState.players.containsKey(secondPlayerId)) {
        Logger().error('Invalid Jack swap - one or both players not found', isOn: loggingSwitch);
        return false;
      }
      ...
      if (firstCard == null || secondCard == null || firstCardIndex == null || secondCardIndex == null) {
        Logger().error('Invalid Jack swap - one or both cards not found in players\' hands', isOn: loggingSwitch);
        return false;
      }
      ...
      Logger().info('Successfully swapped cards: ${firstCard.cardId} <-> ${secondCard.cardId}', isOn: loggingSwitch);
      Logger().info('Player $firstPlayerId now has: ${firstPlayer.hand.map((card) => card?.cardId ?? 'None').toList()}', isOn: loggingSwitch);
      Logger().info('Player $secondPlayerId now has: ${secondPlayer.hand.map((card) => card?.cardId ?? 'None').toList()}', isOn: loggingSwitch);
      
      return true;
      
    } catch (e) {
      Logger().error('Error in _handleJackSwap: $e', isOn: loggingSwitch);
      return false;
    }
  }
  ...
  void endRound() {
    ...
    try {
      ...
      Logger().info('Round $roundNumber ended', isOn: loggingSwitch);
      
    } catch (e) {
      Logger().error('Error ending round: $e', isOn: loggingSwitch);
    }
  }
  ...
  void startNextRound() {
    ...
    try {
      ...
      Logger().info('Starting round $roundNumber', isOn: loggingSwitch);
      
    } catch (e) {
      Logger().error('Error starting next round: $e', isOn: loggingSwitch);
    }
  }
  ...
  bool _handleDrawFromPile(String playerId, Map<String, dynamic> actionData) {
    ...
    try {
      Logger().info("_handle_draw_from_pile called for player $playerId with action_data $actionData", isOn: loggingSwitch);
      ...
      if (source == 'deck') {
        ...
        drawnCard = gameState.drawFromDrawPile();
        if (drawnCard == null) {
          Logger().error("Failed to draw from draw pile for player $playerId", isOn: loggingSwitch);
          return false;
        }
        
        // Check if draw pile is now empty (special game logic)
        if (gameState.isDrawPileEmpty()) {
          Logger().info("Draw pile is now empty", isOn: loggingSwitch);
        }
        ...
        drawnCard = gameState.drawFromDiscardPile();
        if (drawnCard == null) {
          Logger().error("Failed to draw from discard pile for player $playerId", isOn: loggingSwitch);
          return false;
        }
      }
      ...
      if (drawnCard != null) {
        ...
        Logger().error("Failed to draw card for player $playerId", isOn: loggingSwitch);
        return false;
      }
      
      // Change player status from DRAWING_CARD to PLAYING_CARD after successful draw
      player.updateStatus(PlayerStatus.playingCard);
      Logger().info("Player $playerId status changed from DRAWING_CARD to PLAYING_CARD", isOn: loggingSwitch);
      
      // Log pile contents after successful draw using helper methods
      Logger().info("=== PILE CONTENTS AFTER DRAW ===", isOn: loggingSwitch);
      Logger().info("Draw Pile Count: ${gameState.getDrawPileCount()}", isOn: loggingSwitch);
      Logger().info("Draw Pile Top 3: ${gameState.drawPile.take(3).map((card) => card.cardId).toList()}", isOn: loggingSwitch);
      Logger().info("Discard Pile Count: ${gameState.getDiscardPileCount()}", isOn: loggingSwitch);
      Logger().info("Discard Pile Top 3: ${gameState.discardPile.take(3).map((card) => card.cardId).toList()}", isOn: loggingSwitch);
      Logger().info("Drawn Card: ${drawnCard.cardId}", isOn: loggingSwitch);
      Logger().info("=================================", isOn: loggingSwitch);
  ...
  bool _handlePlayCard(String playerId, Map<String, dynamic> actionData) {
    ...
    try {
      ...
      if (cardToPlay == null) {
        Logger().error("Card $cardId not found in player $playerId hand", isOn: loggingSwitch);
        return false;
      }
      
      Logger().info("Found card $cardId at index $cardIndex in player $playerId hand", isOn: loggingSwitch);
      ...
      Logger().info("About to call remove_card_from_hand for card $cardId", isOn: loggingSwitch);
      try {
        final removedCard = player.removeCardFromHand(cardId);
        if (removedCard == null) {
          Logger().error("Failed to remove card $cardId from player $playerId hand", isOn: loggingSwitch);
          return false;
        }
        Logger().info("Successfully removed card $cardId from player $playerId hand", isOn: loggingSwitch);
      } catch (e) {
        Logger().error("Exception in remove_card_from_hand: $e", isOn: loggingSwitch);
        return false;
      }
      
      // Add card to discard pile using custom method with auto change detection
      if (!gameState.addToDiscardPile(cardToPlay)) {
        Logger().error("Failed to add card $cardId to discard pile", isOn: loggingSwitch);
        return false;
      }
      ...
      if (drawnCard != null && drawnCard.cardId != cardId) {
        ...
        Logger().info("Repositioning drawn card ${drawnCard.cardId} to index $cardIndex", isOn: loggingSwitch);
        ...
        if (originalIndex != null) {
          ...
          if (shouldKeepOriginalSlot) {
            ...
            Logger().info("Created blank slot at original position $originalIndex", isOn: loggingSwitch);
          } else {
            player.hand.removeAt(originalIndex); // Remove entirely
            Logger().info("Removed card entirely from original position $originalIndex", isOn: loggingSwitch);
        ...
        if (shouldPlaceInSlot) {
          ...
          Logger().info("Placed drawn card in blank slot at index $cardIndex", isOn: loggingSwitch);
        } else {
          // The slot shouldn't exist, so append the drawn card to the end
          player.hand.add(drawnCard);
          Logger().info("Appended drawn card to end of hand (slot $cardIndex shouldn't exist)", isOn: loggingSwitch);
        }
        ...
        Logger().info("After repositioning: hand slots = ${player.hand.map((card) => card?.cardId ?? 'None').toList()}", isOn: loggingSwitch);
      ...
      Logger().info("=== PILE CONTENTS AFTER PLAY ===", isOn: loggingSwitch);
      Logger().info("Draw Pile Count: ${gameState.drawPile.length}", isOn: loggingSwitch);
      Logger().info("Draw Pile Top 3: ${gameState.drawPile.take(3).map((card) => card.cardId).toList()}", isOn: loggingSwitch);
      Logger().info("Discard Pile Count: ${gameState.discardPile.length}", isOn: loggingSwitch);
      Logger().info("Discard Pile Top 3: ${gameState.discardPile.take(3).map((card) => card.cardId).toList()}", isOn: loggingSwitch);
      Logger().info("Played Card: ${cardToPlay.cardId}", isOn: loggingSwitch);
      Logger().info("=================================", isOn: loggingSwitch);
  ...
  void _checkSpecialCard(String playerId, Map<String, dynamic> cardData) {
    ...
    try {
      ...
      if (cardRank == 'jack') {
        ...
        Logger().info("DEBUG: special_card_data length before adding Jack: ${specialCardData.length}", isOn: loggingSwitch);
        specialCardData.add(specialCardInfo);
        Logger().info("DEBUG: special_card_data length after adding Jack: ${specialCardData.length}", isOn: loggingSwitch);
        Logger().info("Added Jack special card for player $playerId: $cardRank of $cardSuit (chronological order)", isOn: loggingSwitch);
        ...
        Logger().info("DEBUG: special_card_data length before adding Queen: ${specialCardData.length}", isOn: loggingSwitch);
        specialCardData.add(specialCardInfo);
        Logger().info("DEBUG: special_card_data length after adding Queen: ${specialCardData.length}", isOn: loggingSwitch);
        Logger().info("Added Queen special card for player $playerId: $cardRank of $cardSuit (chronological order)", isOn: loggingSwitch);
      }
      
    } catch (e) {
      Logger().error("Error in _check_special_card: $e", isOn: loggingSwitch);
    }
  }
  ...
  bool _handleSameRankWindow(Map<String, dynamic> actionData) {
    ...
    try {
      Logger().info("Starting same rank window - setting all players to SAME_RANK_WINDOW status", isOn: loggingSwitch);
      ...
      Logger().info("Updated $updatedCount players' status to SAME_RANK_WINDOW", isOn: loggingSwitch);
      ...
      Logger().error("Error in _handle_same_rank_window: $e", isOn: loggingSwitch);
      return false;
    }
  }
  ...
  void _endSameRankWindow() {
    ...
    try {
      Logger().info("Ending same rank window - resetting all players to WAITING status", isOn: loggingSwitch);
      
      // Log the same_rank_data before clearing it
      if (sameRankData.isNotEmpty) {
        Logger().info("Same rank plays recorded: ${sameRankData.length} players", isOn: loggingSwitch);
        for (final entry in sameRankData.entries) {
          final playerId = entry.key;
          final playData = entry.value;
          Logger().info("Player $playerId played: ${playData['rank']} of ${playData['suit']}", isOn: loggingSwitch);
        }
      } else {
        Logger().info("No same rank plays recorded", isOn: loggingSwitch);
      }
      ...
      Logger().info("Updated $updatedCount players' status to WAITING", isOn: loggingSwitch);
      ...
      for (final entry in gameState.players.entries) {
        ...
        if (cardCount == 0) {
          Logger().info("Player $playerId (${player.name}) has no cards left - triggering end of match", isOn: loggingSwitch);
          _handleEndOfMatch();
  ...
  void _handleSpecialCardsWindow() {
    ...
    try {
      ...
      if (specialCardData.isEmpty) {
        Logger().info("No special cards played in this round - transitioning directly to ENDING_ROUND", isOn: loggingSwitch);
        // No special cards, go directly to ENDING_ROUND
        gameState.phase = GamePhase.endingRound;
        Logger().info("Game phase changed to ENDING_ROUND (no special cards)", isOn: loggingSwitch);
      ...
      Logger().info("Game phase changed to SPECIAL_PLAY_WINDOW (special cards found)", isOn: loggingSwitch);
      
      Logger().info("=== SPECIAL CARDS WINDOW ===", isOn: loggingSwitch);
      Logger().info("DEBUG: special_card_data length: ${specialCardData.length}", isOn: loggingSwitch);
      Logger().info("DEBUG: Current game phase: ${gameState.phase.name}", isOn: loggingSwitch);
      
      // Count total special cards (now stored chronologically)
      final totalSpecialCards = specialCardData.length;
      Logger().info("Found $totalSpecialCards special cards played in chronological order", isOn: loggingSwitch);
      ...
      for (int i = 0; i < specialCardData.length; i++) {
        ...
        Logger().info("  ${i+1}. Player ${card['player_id']}: ${card['rank']} of ${card['suit']} (${card['special_power']})", isOn: loggingSwitch);
      }
      
      // Create a working copy for processing (we'll remove cards as we process them)
      specialCardPlayers = List.from(specialCardData);
      
      Logger().info("Starting special card processing with ${specialCardPlayers.length} cards", isOn: loggingSwitch);
               
      // Start processing the first player's special card
      _processNextSpecialCard();
      
    } catch (e) {
      Logger().error("Error in _handle_special_cards_window: $e", isOn: loggingSwitch);
    }
  }
  ...
  void _processNextSpecialCard() {
    ...
    try {
      ...
      if (specialCardPlayers.isEmpty) {
        Logger().info("All special cards processed - transitioning to ENDING_ROUND", isOn: loggingSwitch);
      ...
      Logger().info("Processing special card for player $playerId: $cardRank of $cardSuit", isOn: loggingSwitch);
      Logger().info("  Special Power: $specialPower", isOn: loggingSwitch);
      Logger().info("  Description: $description", isOn: loggingSwitch);
      Logger().info("  Remaining cards to process: ${specialCardPlayers.length}", isOn: loggingSwitch);
      ...
      if (specialPower == 'jack_swap') {
        ...
        Logger().info("Player $playerId status set to JACK_SWAP - 10 second timer started", isOn: loggingSwitch);
      } else if (specialPower == 'queen_peek') {
        // Use the efficient batch update method to set player status
        gameState.updatePlayersStatusByIds([playerId], PlayerStatus.queenPeek);
        Logger().info("Player $playerId status set to PEEKING - 10 second timer started", isOn: loggingSwitch);
      } else {
        Logger().info("Unknown special power: $specialPower for player $playerId", isOn: loggingSwitch);
      ...
      Logger().info("10-second timer started for player $playerId's $specialPower", isOn: loggingSwitch);
      
    } catch (e) {
      Logger().error("Error in _process_next_special_card: $e", isOn: loggingSwitch);
    }
  }
  ...
  void _onSpecialCardTimerExpired() {
    ...
    try {
      ...
      if (specialCardPlayers.isNotEmpty) {
        ...
        Logger().info("Player $playerId special card timer expired - status reset to WAITING", isOn: loggingSwitch);
        
        // Remove the processed card from the list
        specialCardPlayers.removeAt(0);
        Logger().info("Removed processed card from list. Remaining cards: ${specialCardPlayers.length}", isOn: loggingSwitch);
      }
      ...
      Logger().error("Error in _on_special_card_timer_expired: $e", isOn: loggingSwitch);
    }
  }
  ...
  void _endSpecialCardsWindow() {
    ...
    try {
      ...
      if (specialCardData.isNotEmpty) {
        specialCardData.clear();
        Logger().info("Special card data cleared", isOn: loggingSwitch);
      }
      ...
      Logger().error("Error in _end_special_cards_window: $e", isOn: loggingSwitch);
    }
  }
  ...
  void cancelSpecialCardTimer() {
    ...
    try {
      if (specialCardTimer != null) {
        ...
        Logger().info("Special card timer cancelled", isOn: loggingSwitch);
      }
    } catch (e) {
      Logger().error("Error cancelling special card timer: $e", isOn: loggingSwitch);
    }
  }
  ...
  bool _handleSameRankPlay(String userId, Map<String, dynamic> actionData) {
    ...
    try {
      ...
      if (playedCard == null) {
        Logger().error("Card $cardId not found in player $userId hand for same rank play", isOn: loggingSwitch);
        return false;
      }
      
      Logger().info("Found card $cardId for same rank play in player $userId hand", isOn: loggingSwitch);
      ...
      // Use the proper method to remove card with change detection
      Logger().info("About to call remove_card_from_hand for same rank play card $cardId", isOn: loggingSwitch);
      try {
        final removedCard = player.removeCardFromHand(cardId);
        if (removedCard == null) {
          Logger().error("Failed to remove card $cardId from player $userId hand", isOn: loggingSwitch);
          return false;
        }
        Logger().info("Successfully removed same rank play card $cardId from player $userId hand", isOn: loggingSwitch);
      } catch (e) {
        Logger().error("Exception in remove_card_from_hand for same rank play: $e", isOn: loggingSwitch);
        return false;
      }
      ...
      Logger().info("✅ Same rank play successful: $userId played $cardRank of $cardSuit - card moved to discard pile", isOn: loggingSwitch);
  ...
  bool _validateSameRankPlay(String cardRank) {
    ...
    try {
      ...
      if (gameState.discardPile.isEmpty) {
        Logger().info("Same rank validation failed: No cards in discard pile", isOn: loggingSwitch);
        return false;
      }
      ...
      Logger().info("Same rank validation: played_card_rank='$cardRank', last_card_rank='$lastCardRank'", isOn: loggingSwitch);
      
      // Handle special case: first card of the game (no previous card to match)
      if (gameState.discardPile.length == 1) {
        Logger().info("Same rank validation: First card of game, allowing play", isOn: loggingSwitch);
        return true;
      }
      
      // Check if ranks match (case-insensitive for safety)
      if (cardRank.toLowerCase() == lastCardRank.toLowerCase()) {
        Logger().info("Same rank validation: Ranks match, allowing play", isOn: loggingSwitch);
        return true;
      } else {
        Logger().info("Same rank validation: Ranks don't match, denying play", isOn: loggingSwitch);
        return false;
      }
        
    } catch (e) {
      Logger().error("Same rank validation error: $e", isOn: loggingSwitch);
      return false;
    }
  }
  ...
  Card? _applySameRankPenalty(String playerId) {
    ...
    try {
      ...
      if (penaltyCard == null) {
        Logger().error("Failed to draw penalty card from draw pile for player $playerId", isOn: loggingSwitch);
        return null;
      }
      ...
      Logger().info("Player $playerId status reset to WAITING after penalty", isOn: loggingSwitch);
  ...
  void _handleEndOfMatch() {
    ...
    try {
      ...
      if (winnerData['is_tie'] == true) {
        ...
        for (final winnerName in winners) {
          for (final entry in gameState.players.entries) {
            ...
            if (player.name == winnerName) {
              ...
              Logger().info("Player ${player.name} set to FINISHED status (tie)", isOn: loggingSwitch);
            }
          }
        }
      } else {
        ...
        for (final entry in gameState.players.entries) {
          ...
          if (entry.key != winnerId) {
            player.updateStatus(PlayerStatus.finished);
            Logger().info("Player ${player.name} set to FINISHED status", isOn: loggingSwitch);
          }
        }
      }
      ...
      Logger().error("Error in _handle_end_of_match: $e", isOn: loggingSwitch);
    }
  }
  ...
  void dispose() {
    ...
    try {
      ...
      Logger().info('GameRound disposed', isOn: loggingSwitch);
      
    } catch (e) {
      Logger().error('Error disposing GameRound: $e', isOn: loggingSwitch);
    }
  }
}