...
class YamlRulesEngine {
  ...
  String executeRules(List<dynamic> rules, Map<String, dynamic> gameData, bool shouldPlayOptimal) {
    Logger().info('Recall: DEBUG - YamlRulesEngine.executeRules called with ${rules.length} rules, shouldPlayOptimal: $shouldPlayOptimal', isOn: LOGGING_SWITCH);
    
    // Sort rules by priority
    final sortedRules = List<Map<String, dynamic>>.from(rules)
      ..sort((a, b) => (a['priority'] ?? 999).compareTo(b['priority'] ?? 999));
    
    Logger().info('Recall: DEBUG - Sorted rules by priority: ${sortedRules.map((r) => '${r['name']} (${r['priority']})').join(', ')}', isOn: LOGGING_SWITCH);
    ...
    if (!shouldPlayOptimal && sortedRules.isNotEmpty) {
      ...
      Logger().info('Recall: DEBUG - Not playing optimally, using last rule: ${lastRule['name']}', isOn: LOGGING_SWITCH);
    ...
    for (final rule in sortedRules) {
      ...
      Logger().info('Recall: DEBUG - Evaluating rule: $ruleName', isOn: LOGGING_SWITCH);
      
      final condition = rule['condition'] as Map<String, dynamic>?;
      if (condition != null) {
        final conditionResult = _evaluateCondition(condition, gameData);
        Logger().info('Recall: DEBUG - Rule $ruleName condition result: $conditionResult', isOn: LOGGING_SWITCH);
        
        if (conditionResult) {
          final action = rule['action'] as Map<String, dynamic>?;
          if (action != null) {
            Logger().info('Recall: DEBUG - Rule $ruleName condition passed, executing action', isOn: LOGGING_SWITCH);
    ...
    Logger().info('Recall: DEBUG - No rules matched, using fallback logic', isOn: LOGGING_SWITCH);
    
    // Ultimate fallback: random from playable cards
    final playableCards = gameData['playable_cards'] as List<dynamic>? ?? [];
    if (playableCards.isNotEmpty) {
      Logger().info('Recall: DEBUG - Using playable cards fallback: ${playableCards.length} cards', isOn: LOGGING_SWITCH);
    ...
    Logger().info('Recall: DEBUG - Using available cards fallback: ${availableCards.length} cards', isOn: LOGGING_SWITCH);
  ...
  String _executeAction(Map<String, dynamic> action, Map<String, dynamic> gameData) {
    ...
    Logger().info('Recall: DEBUG - _executeAction called with type: $type, source: $source, filters: ${filters.length}', isOn: LOGGING_SWITCH);
    
    // Get source data
    List<dynamic> sourceData = gameData[source] as List<dynamic>? ?? [];
    Logger().info('Recall: DEBUG - Initial source data from $source: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    ...
    Logger().info('Recall: DEBUG - Source data after null/collection filtering: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    
    // Apply filters
    for (final filter in filters) {
      if (filter is Map<String, dynamic>) {
        Logger().info('Recall: DEBUG - Applying filter: ${filter['type']}', isOn: LOGGING_SWITCH);
        sourceData = _applyFilter(sourceData, filter, gameData);
        Logger().info('Recall: DEBUG - After filter, source data: ${sourceData.length} items', isOn: LOGGING_SWITCH);
      }
    }
    ...
    if (sourceData.isEmpty) {
      ...
      Logger().info('Recall: DEBUG - Source data empty, using playable cards fallback: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    ...
    if (sourceData.isEmpty) {
      ...
      Logger().info('Recall: DEBUG - Still empty, using available cards fallback: ${sourceData.length} items', isOn: LOGGING_SWITCH);
    ...
    if (sourceData.isEmpty) {
      Logger().warning('Recall: DEBUG - All fallbacks failed, no cards available!', isOn: LOGGING_SWITCH);
    ...
    switch (type) {
      case 'select_random':
        ...
        Logger().info('Recall: DEBUG - select_random result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_highest_points':
        result = _selectHighestPoints(sourceData, gameData);
        Logger().info('Recall: DEBUG - select_highest_points result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_lowest_points':
        result = _selectLowestPoints(sourceData, gameData);
        Logger().info('Recall: DEBUG - select_lowest_points result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_first':
        result = sourceData.first.toString();
        Logger().info('Recall: DEBUG - select_first result: $result', isOn: LOGGING_SWITCH);
        break;
      
      case 'select_last':
        result = sourceData.last.toString();
        Logger().info('Recall: DEBUG - select_last result: $result', isOn: LOGGING_SWITCH);
        break;
      
      default:
        result = sourceData[_random.nextInt(sourceData.length)].toString();
        Logger().info('Recall: DEBUG - default action result: $result', isOn: LOGGING_SWITCH);
        break;
    }
