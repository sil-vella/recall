#!/usr/bin/env python3
"""
Upload Table Logo Image Script
Reads the table logo image from app_media/media/table_logo.webp (relative to project root)
and uploads it to the VPS at /var/www/dutch.reignofplay.com/app_media/media/table_logo.webp (served at https://dutch.reignofplay.com/...).
...
REMOTE_IMAGE_DIR = '/var/www/dutch.reignofplay.com/app_media/media'
REMOTE_IMAGE_PATH = f'{REMOTE_IMAGE_DIR}/table_logo.webp'
REMOTE_TMP_IMAGE = '/tmp/table_logo.webp'
LOCAL_IMAGE_PATH = PROJECT_ROOT / 'app_media' / 'media' / 'table_logo.webp'
...
def upload_image(local_image_path: Path):
    ...
    print(f"Expected URL: {Colors.BLUE}https://dutch.reignofplay.com/app_media/media/table_logo.webp{Colors.NC}")
    print()
...
def main():
    """Main function."""
    print(f"{Colors.BLUE}=== Table Logo Image Upload Script ==={Colors.NC}\n")
    
    # Check if SSH key exists
    if not check_ssh_key():
        sys.exit(1)
    
    # Image is always taken from app_media/media/table_logo.webp
