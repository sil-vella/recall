...
Usage:
  ...
  python playbooks/rop01/16_upload_card_back_packs.py --pack juventus
"""
...
def find_pack_files(pack: str | None, upload_all: bool) -> List[Path]:
    if not LOCAL_BASE_DIR.exists():
        return []
    if upload_all:
        files = sorted(LOCAL_BASE_DIR.glob("*/card_back_*.webp"))
        return [f for f in files if f.is_file()]
    if not pack:
        return []
    pack_name = pack.strip().lower()
    expected = LOCAL_BASE_DIR / pack_name / f"card_back_{pack_name}.webp"
...
def main():
    args = parse_args()
    print(f"{Colors.BLUE}=== Card Back Pack Upload ==={Colors.NC}")
    ...
    if not files:
        print(f"{Colors.RED}No matching card-back files found.{Colors.NC}")
        sys.exit(1)
