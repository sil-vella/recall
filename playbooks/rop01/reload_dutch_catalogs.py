#!/usr/bin/env python3
"""
Deploy / VPS: hot-reload Dutch declarative catalogs on a running Python API (no Flask restart).
...
Usage:
  python playbooks/rop01/reload_dutch_catalogs.py
  python playbooks/rop01/reload_dutch_catalogs.py --url https://api.example.com

Local Mac dev: use playbooks/00_local/reload_dutch_catalogs.py instead.

Env (repo-root .env.local loaded automatically when present):
  DART_BACKEND_SERVICE_KEY   — preferred X-Service-Key
  DUTCH_MT_DASHBOARD_SERVICE_KEY — alternate key accepted by Python
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="POST reload-catalogs to running Dutch Python API",
    )
    parser.add_argument(
        "--url",
