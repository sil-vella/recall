...
class ClecoGameMain(BaseModule):
    ...
    def register_routes(self):
        ...
        try:
            custom_log("🔐 ClecoGame: Starting route registration", level="INFO", isOn=True)
            
            # Import and register API blueprint
            from .api_endpoints import cleco_api
            custom_log("🔐 ClecoGame: Imported API blueprint", level="INFO", isOn=True)
            
            self.app.register_blueprint(cleco_api)
            custom_log("🔐 ClecoGame: API blueprint registered successfully", level="INFO", isOn=True)
            ...
            custom_log("🔐 ClecoGame: All routes registered successfully", level="INFO", isOn=True)
            return True
        except Exception as e:
            custom_log(f"❌ ClecoGame: Error registering routes: {e}", level="ERROR", isOn=True)
            return False
