...
impl GameStateManager {
    ...
    pub fn _to_flutter_game_data(&self, game: &GameState) -> serde_json::Value {
    ...
    pub fn _to_flutter_card(&self, card: &Card) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "rank": rank_mapping.get(card.rank.to_string().as_str()).unwrap_or(&card.rank.to_string().as_str()),
            "points": card.points,
    ...
    pub fn _to_flutter_player_data(&self, player: &Player, is_current: bool) -> serde_json::Value {
        serde_json::json!({
            "id": player.player_id,
            "name": player.name,
