...
class InAppPurchasesModule:
    ...
    def initialize(self, app_initializer):
        ...
        # Initialize database schema (without mock data sync)
    ...
    def register_routes(self):
        ...
        try:
            ...
            # Register routes
            self.blueprint.route('/verify-purchase', methods=['POST'])(self.verify_purchase)
            self.blueprint.route('/purchase-history', methods=['GET'])(self.get_purchase_history)
            self.blueprint.route('/restore-purchases', methods=['POST'])(self.restore_purchases)
            self.blueprint.route('/sync-products', methods=['POST'])(self.sync_products)
            self.blueprint.route('/products', methods=['GET'])(self.get_products)
            self.blueprint.route('/sync-history', methods=['GET'])(self.get_sync_history)
            ...
            custom_log(f"❌ Failed to register {self.module_name} routes: {e}", level="ERROR")
    ...
    def _update_user_module_data(self, user_id: str, product_id: str, verification_result: Dict[str, Any]):
        ...
        try:
            ...
            db_manager.update("users", {"_id": user_id}, module_update)
    ...
    def clear_mock_data(self):
        ...
        try:
            ...
            custom_log(f"🗑️ Deleted {result.deleted_count} mock products", level="INFO")
            
            # Clear sync_history collection
            result = db_manager.db.sync_history.delete_many({})
            custom_log(f"🗑️ Deleted {result.deleted_count} mock sync records", level="INFO")
            ...
            custom_log(f"❌ Failed to clear mock data: {e}", level="ERROR")
            return jsonify({
                "success": False,
                "error": str(e)
            }), 500
